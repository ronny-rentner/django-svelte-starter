...
The Django template shell loads the Svelte entrypoint through `django-vite` using `static/frontend/manifest.json`. `static/frontend/` is generated by `./dm build front`, which wraps `cd frontend && npm run build`. `static/src/` is Django-side static source, such as admin styling, favicon, email/logo assets, and other non-bundled files; it is not Svelte build output. `frontend/src/build-info.json` is untracked and counted up by svUltra's `updateBuildInfo.js`, the `prebuild` script; the footer reads it through `svultra/kit/buildInfo`, which tolerates its absence. Do not hand-edit it.
