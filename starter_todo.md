...
- Bug, hot reloading: when `app.css` references font files before they exist, Vite's dev server keeps the unresolved `url()` even after the files appear and `app.css` changes again; the fonts fail until the dev server is restarted. Hot reloading has to pick the files up.
- Caching after a deploy: the browser kept showing the old logo and fonts of aylinschaer.de until a forced reload. Cause: `immutable_file_test` (djultra `settings.py:316`, the same in Relonee `settings.py:779`) marks every `jpg`, `svg`, `png`, `woff`, `woff2` and `txt` as immutable by extension, so they are sent with `cache-control: max-age=315360000, public, immutable`, while the build names them without a hash (`assetFileNames: 'assets/[name].[ext]'`, also from Relonee) and a changed file keeps its URL. Relonee's comment says the test "used to" match vite's hashes. nginx adds no cache headers. CSS and JS get `max-age=60` with an ETag.
- `dm docker deploy` never updates an unpinned git dependency: `pyproject.toml` is unchanged, so Docker reuses the cached pip layer and the image keeps the old djultra. `--no-cache` works but reinstalls and re-downloads everything. We need a way to check for and install new packages without that.
- `PROJECT_NAME = BASE_DIR.name` is `app` inside the container (`COPY . /app`), so the shell's `<title>` and the admin's name are wrong in production. Part of the one-variable-for-the-domain item.
- The sites still ship their own `backend/core/templates/index.html`, which overrides djultra's; delete it with the next starter merge, carbon.berlin's too.
...
- We cannot install a single npm package without npm enforcing `package.json`. Find a way to stop npm from doing that. With npm 12 it now fails outright: `npm install @fontsource/oswald` re-resolves `svultra@github:…` and aborts with `EALLOWGIT: Fetching packages of type "git" have been disabled`. `npm pack <pkg>` plus `tar` gets a package's files without touching the tree.
