...
from .authentication import IsOwner
...
class PersonView(APIView):
    ...
    serializer_class = serializers.DynamicFieldsModelSerializer

    def get_permissions(self):
        permissions = super().get_permissions() + [IsOwner(owner_pk_field='pk')]
        return permissions

    def get_person(self, request, person_pk=None):
        if person_pk:
            return Person.objects.filter(pk=person_pk).first()
        else:
            return Person.objects.filter(pk=request.user.person.pk).first()

    def patch(self, request, person_pk=None):
        person = self.get_person(request, person_pk)

        self.check_object_permissions(request, person)

        serializer = self.serializer_class(person, data=request.data, partial=True, model=Person)
    ...
    def get(self, request, person_pk=None):
        person = self.get_person(request, person_pk)
        ...
        logger.debug('Person API view: ', person)
