<script>
  ...
  import externalLinkIcon from '@iconify-icons/ph/arrow-square-out-duotone';


  import { configStore as config } from '@stores';

  import { submitTermsForm, signOut, fetchUserInfo } from '@api/api.js';
  ...
  let { open, dialog = $bindable(), children, ...restProps } = $props();
  ...
  async function onsubmit(event) {
    ...
    try {
      ...
      const response = await submitTermsForm(true);
      if (response.error) {
...
<style>
  ...
  Dialog {
    ...
    div.text {
      ...
      border: var(--pico-outline-width) solid var(--pico-form-element-border-color);
...
<Dialog bind:this={dialog} {onsubmit} novalidate noclose {open} {...restProps}>
  ...
  <p>
    You have been invited to Relonee's <i>Premium Relocation Support</i> package to help you with your relocation to Germany. Please confirm this invitation and agree to our terms below to allow us working with you on your relocation.
    <Link href="/" target="_blank" icon={externalLinkIcon}>Learn more about Relonee.</Link>
  </p>

  <Accordion header:class="terms">
    {snippet header()}Terms &amp; Conditions{/snippet}
    <div class="text" tabindex="0">
      <markdown file="Terms.md" />
    </div>
  </Accordion>
  <Accordion header:class="terms">
    {snippet header()}Privacy Policy{/snippet}
    <div class="text" tabindex="0">
      <markdown file="Privacy.md" />
    </div>
  </Accordion>
