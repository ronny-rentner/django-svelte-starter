      ...
      output: {
        ...
        chunkFileNames: (chunkInfo) => {
          if (chunkInfo.facadeModuleId) {
            const componentName = chunkInfo.facadeModuleId
              .split('/')
              .pop()
              .replace(/\.[^/.]+$/, ''); // Extract component name and remove extension
            return `assets/[name]-${componentName}.js`; // Custom chunk name based on the component
          }
          return `assets/[name]-[hash].js`; // Fallback for non-facade chunks
        },
        */
